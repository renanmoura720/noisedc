...
def test_evento_normal_nao_e_anomalo():
    evento = Evento(equipamento="EV11", estado=0, confianca=0.95)
    assert not evento.anomalo
    assert evento.severidade() == "informativo"


def test_severidade_cresce_com_a_confianca():
    baixa = Evento("EV11", 1, 0.60).severidade(confianca_minima=0.80)
    media = Evento("EV11", 1, 0.85).severidade(confianca_minima=0.80)
    alta = Evento("EV11", 1, 0.99).severidade(confianca_minima=0.80)
    assert (baixa, media, alta) == ("aviso", "medio", "alto")


def test_dicionario_do_evento_tem_rastreabilidade():
    dados = Evento("EV11", 1, 0.9, arquivo_audio="a.wav", dispositivo="snitch-01").como_dicionario()
...
def test_modo_de_simulacao_nao_exige_servidor():
    ...
    comandos = cliente.enviar_evento(Evento("EV11", 1, 0.9))
...
def test_arquivo_de_lote_tem_duas_linhas_por_evento(tmp_path):
    destino = escrever_lote(
        [Evento("EV11", 1, 0.9), Evento("EV12", 0, 0.8)], tmp_path / "lote.txt"
    )
    linhas = destino.read_text(encoding="utf-8").strip().splitlines()
    assert len(linhas) == 4
    assert linhas[0].startswith("EV11 acustico.estado")
